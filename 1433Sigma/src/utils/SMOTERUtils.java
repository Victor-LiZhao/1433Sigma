...
public class SMOTERUtils {
	 ...
	 * @param dataSet  原始数据集
	 * @param tempArray 需要测试的样本
	 * @param dem  样本维度
	 * @param threshold  阈值
	 * @param over  过采用比例的100倍  %over   如果一个样本点过采用5倍生成5个则over=500
	 * @param under 欠采样比例
	 * @param k knn参数 k
	 * @return 新的数据集合
	 */
	public static List<Double[]> getSmoteR(List<Double[]> dataSet,Double[] tempArray, int dem,Double threshold,int over,int under,int k){
	 ...
	 * @return归一化之后的数据集
	 */
	private static List<Double[]> propertyToOne(List<Double[]> dataSet, Double[] tempArray, int dem) {
		//初始化最大最小数组
	...
	private static List<Double[]> genSynthCases(List<Double[]> rareList,int dem, int ng,
			int k) {
